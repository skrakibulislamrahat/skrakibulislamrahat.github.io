...
function breakdown(t) {
  return '<div class="breakdown"><div><span>Hourly pay · '+duration(t.minutes)+'</span><strong>'+money(t.wages)+'</strong></div><div><span>Item commissions</span><strong>'+money(t.items)+'</strong></div><div><span>Daily sales bonus</span><strong>'+money(t.bonus)+'</strong></div></div>';
}
function render() {
  if(!data)return;
...
function renderUnpaid() {
  ...
  html+='<div class="mini-stats"><div class="mini-stat"><span>Unpaid total</span><strong>'+money(t.total)+'</strong></div><div class="mini-stat"><span>Completed hours</span><strong>'+duration(t.minutes)+'</strong></div><div class="mini-stat"><span>Commissions + bonus</span><strong>'+money(t.items+t.bonus)+'</strong></div></div>';
...
async function saveDay(form,edit=false) {
  ...
  try {
    ...
    changed.sales=readMoney(fields,'sales');changed.counts=readCounts(fields);changed.note=String(fields.get('note')||'');
    if(edit) {
...
document.addEventListener('input',event=>{
  ...
  if(form?.id==='day-form'){
    ...
    draft.sales=Math.max(0,Math.round(Number(fields.get('sales'))*100)||0);draft.counts=readCounts(fields);
